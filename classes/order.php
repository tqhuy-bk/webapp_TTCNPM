 ...
 <?php 
    class order
    {
      ...
      public function add_order($customer_id){//thanh toan bang tai khoan ung dung
        ...
        public function show_order_list_unpaid(){           
            $vendorid = Session::get('vendorid');
            $query = "SELECT * FROM tbl_order WHERE paystatus ='Chưa thanh toán' order by orderID desc";
            $result = $this->db->select($query);
            return $result;
        }
        public function show_order_list_undone(){           
            $vendorid = Session::get('vendorid');
            $query = "SELECT * FROM tbl_order WHERE paystatus ='Đã thanh toán' AND state='processing' order by orderID desc";
            $result = $this->db->select($query);
            return $result;
        }
    }
